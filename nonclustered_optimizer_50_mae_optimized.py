# -*- coding: utf-8 -*-
"""nonclustered_optimizer_50_MAE_optimized.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1Yx4sLCkgOJuL9DUfhqDbbMhG26P469Yv
"""
...
def grid_search(param_grid, data):
    ...
    if len(all_params) < 1000:
        with ThreadPoolExecutor(max_workers=8) as executor:
            futures = [executor.submit(calculate_mae, params, data) for params in all_params]
            for future in as_completed(futures):
                mae, params = future.result()
        ...
        for i in range(0, len(all_params), batch_size):
            ...
            for params in batch:
                mae, _ = calculate_mae(params, data)
                results.append((mae, params))
...
def optimizer(data):
    ...
    param_grid = [alpha_range, beta_range, gamma_range]
    mae, (alpha, beta, gamma) = grid_search(param_grid, data)

    # Generate final forecasts
    forecasts, slope = model_w_parameters(alpha, beta, gamma, data)

    return pd.DataFrame({'Forecast': forecasts, 'Actual_Observation': data}), slope, alpha, beta, gamma, mae
...
def excel_writer(izmir_poligon_path , ilce):
  ...
  final_df.to_excel(izmir_poligon_path + '/' + ilce + '/' +ilce+'_errors_params-50-MAE-optimized.xlsx')
