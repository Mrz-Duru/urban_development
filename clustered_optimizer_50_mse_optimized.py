# -*- coding: utf-8 -*-
"""clustered_optimizer_50_MSE_optimized.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/12NXehx_pCriGmbnqp0V2Ch6tprdklwwy
"""
...
def calculate_mse(params, dataframe, colnames):
    ...
    for i, colname in enumerate(colnames):
        ...
        if np.any(valid_indices):
            mse = np.mean(((actual[valid_indices] - forecast[valid_indices]))**2 )
            mses.append(mse)

    return np.mean(mses) if mses else float('inf')
...
def optimizer_in_clusters(dataframe, n_jobs=-1, grid_sizes=(10, 5, 3)):
    ...
    with ProcessPoolExecutor(max_workers=n_jobs) as executor:
        mse_func = functools.partial(calculate_mse, dataframe=dataframe, colnames=colnames)
        results = list(executor.map(mse_func, param_grid))
    ...
    return best_params[0], best_params[1], best_params[2], best_mse
...
def writer(zone_info , parameter_info , error_info ,root  , ilce ,column_order ):
  for elements in zone_info:
    dict_of_lists = {'ZoneId': [] , 'alpha' : [] , 'beta': [] , 'gamma': [] , 'train_mse' : [] , 'test_mape' : [] , 'test_mse' :[] , 'test_mae' : [] }
    for cluster_number in zone_info[elements]:
      for i , zone in enumerate(zone_info[elements][cluster_number]):
        ...
        dict_of_lists['train_mse'].append(parameter_info[elements][cluster_number][3])
    ...
    df.to_excel(root + '/' + ilce + '/' + ilce + '_' + elements + '_errors_params-50-MSE-optimized.xlsx')
